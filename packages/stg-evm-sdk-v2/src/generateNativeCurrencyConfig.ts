...
function generateNativeCurrencyConfig(metadata: LayerZeroApiResponse): NativeCurrencyConfig {
    ...
    for (const [chainName, chainData] of Object.entries(metadata)) {
        try {
            ...
            if (!nativeCurrency) {
                console.warn(`⚠️  Skipping ${chainName}: missing nativeCurrency`)
                continue
            }

            if (typeof nativeCurrency.decimals !== 'number' || !nativeCurrency.symbol) {
                console.warn(`⚠️  Skipping ${chainName}: invalid nativeCurrency data`, nativeCurrency)
                continue
            }
...
function writeConfigToFile(config: NativeCurrencyConfig, outputPath: string): void {
    ...
    console.log(`✅ Successfully wrote ${Object.keys(config).length} chain configurations`)
}

async function main() {
    try {
        console.log('🚀 Starting native currency configuration generation...\n')
        ...
        console.error('❌ Error generating native currency configuration:', error)
        process.exit(1)
    }
}
