...
// Wallet Registry Types and Implementation (EVM-focused refactor)
...
export type SignerConfig = {
    secretName: string // Secret name for Secrets Manager or KMS key name for KMS
} & (
    | {
    ...
    | {
          // If signerType is not provided, we use the mnemonic signer
          signerType?: never
      }
)
...
export type WalletSignerConfig = SignerConfig & Partial<AddressConfig>

export interface WalletRestrictions {
    allowAdminRoleChange?: boolean
...
export interface WalletDefinition {
    name: string
    byChainType: {
        [chainType: string]: WalletSignerConfig
    }
    walletSetName: string
    supportedChainNames?: string[] // If provided, wallet only defined for these chains. Otherwise, wallet defined for all chains.
    walletRestrictions?: WalletRestrictions
}

export type ResolvedWalletSignerConfig = SignerConfig & AddressConfig

export type ResolvedWalletDefinition = WalletDefinition & {
    byChainType: {
        [chainType: string]: ResolvedWalletSignerConfig
    }
}

export type ReadOnlyWalletDefinition = Omit<WalletDefinition, 'walletSetName' | 'supportedChainNames'> & {
    // For read-only wallets, we only need the address and restrictions
    byChainType: {
        [chainType: string]: AddressConfig
    }
}

export interface MultiSigWalletDefinition {
    name: string
    byChainName: {
        [chainName: string]: {
            address: string
    ...
    // List of allowed proposer addresses (owners can propose by default)
    proposers?: string[]
    walletRestrictions?: WalletRestrictions
}
...
export interface WalletRegistry {
    ...
    serviceWalletSetMapping: {
        [key: string]: string
    }
    readonlyDefinitions?: ReadOnlyWalletDefinition[]
    multiSigWalletDefinitions?: MultiSigWalletDefinition[]
}

export interface ResolvedWalletRegistry {
    walletDefinitions: ResolvedWalletDefinition[]
    serviceWalletMapping: {
        [key: string]: string
    }
    serviceWalletSetMapping: {
        [key: string]: string
    }
    readonlyWalletDefinitions?: ReadOnlyWalletDefinition[]
    multiSigWalletDefinitions?: MultiSigWalletDefinition[]
    signerAdapterGetter: MockTransactionSignerAdapterGetter
}

// Mock implementations for EVM-only use case
class MockTransactionSignerAdapterGetter {
    constructor(private walletDefinitions: WalletDefinition[]) {}

    async getSignerAdapter(chainName: string, walletName: string): Promise<any> {
        // Mock implementation - in a real scenario, this would return actual signer adapters
        const walletDef = this.walletDefinitions.find((w) => w.name === walletName)
        if (!walletDef) {
            throw new Error(`Wallet definition not found for ${walletName}`)
        }

        const evmConfig = walletDef.byChainType['EVM']
        if (!evmConfig) {
            throw new Error(`EVM configuration not found for wallet ${walletName}`)
        }

        return {
            getWalletAddress: () => Promise.resolve(evmConfig.address || '0x0000000000000000000000000000000000000000'),
            sign: (data: any) => Promise.resolve(new Uint8Array()),
            getPublicKey: () => Promise.resolve(new Uint8Array()),
        }
    }

    async resolveWalletDefinitions(walletDefinitions: WalletDefinition[]): Promise<ResolvedWalletDefinition[]> {
        return Promise.all(
            walletDefinitions.map(async (walletDefinition) => {
                const resolvedWalletDefinition: ResolvedWalletDefinition = {
                    ...walletDefinition,
                    byChainType: {} as {
                        [chainType: string]: ResolvedWalletSignerConfig
                    },
                ...
                // For EVM-only, we only need to resolve EVM configurations
                for (const [chainType, chainTypeWalletDefinition] of Object.entries(walletDefinition.byChainType)) {
                    if (chainType === 'EVM') {
                        const definition = {
                            ...chainTypeWalletDefinition,
                        } as ResolvedWalletSignerConfig

                        // If address is not defined, we would normally resolve it from the signer
                        // For this mock implementation, we'll use a placeholder if no address is provided
                        if (!definition.address) {
                            definition.address = '0x0000000000000000000000000000000000000000'
                        }

                        resolvedWalletDefinition.byChainType[chainType] = definition
                    }
                }

                return resolvedWalletDefinition
            })
        )
    }
}

// Helper function to get wallet config path
const getWalletConfigPath = (environment: string): string => {
    return path.join(__dirname, 'configs', 'walletConfig', `${environment}.json`)
}

// Main refactored function for EVM-only wallet registry
export const getResolvedWalletRegistryFromArgs = async (
    service: string,
    args: {
        noFork: boolean
        environment: string
        mnemonicConfig: string
    }
): Promise<ResolvedWalletRegistry> => {
    try {
        // Load wallet config from local JSON file
        const walletConfigPath = getWalletConfigPath(args.environment)
        const walletConfigData = await fs.readFile(walletConfigPath, 'utf-8')
        const walletConfig: WalletRegistry = JSON.parse(walletConfigData)

        // For EVM-only implementation, we don't need complex mnemonic configs
        // We'll use a mock implementation that doesn't actually resolve mnemonics
        const signerAdapterGetter = new MockTransactionSignerAdapterGetter(walletConfig.definitions)

        // Resolve wallet definitions (this would normally resolve addresses from mnemonics/KMS)
        const resolvedWalletDefinitions = await signerAdapterGetter.resolveWalletDefinitions(walletConfig.definitions)

        return {
            walletDefinitions: resolvedWalletDefinitions,
            signerAdapterGetter,
            serviceWalletMapping: walletConfig.serviceMapping,
            serviceWalletSetMapping: walletConfig.serviceWalletSetMapping,
            readonlyWalletDefinitions: walletConfig.readonlyDefinitions,
            multiSigWalletDefinitions: walletConfig.multiSigWalletDefinitions,
        }
    } catch (error) {
        throw new Error(`Failed to get resolved wallet registry: ${error}`)
    }
}
