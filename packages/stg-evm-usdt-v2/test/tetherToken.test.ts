...
describe('TetherToken', async () => {
    ...
    describe('Mint & Transfer', async () => {
        ...
        it('multiTransfer: tokens can be transferred to multiple addresses to save gas', async () => {
            const { token, owner, otherAccount, thirdAccount } = await loadFixture(deployFreshTether)

            await token.connect(otherAccount).multiTransfer([owner.getAddress(), thirdAccount.getAddress()], [200, 300])
        ...
        it('multiTransfer: throws error if array lengths do not match', async () => {
            ...
            const tx = token.connect(otherAccount).multiTransfer([owner.getAddress(), thirdAccount.getAddress()], [200])
