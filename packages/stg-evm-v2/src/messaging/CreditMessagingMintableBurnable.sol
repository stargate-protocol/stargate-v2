...
import { ICreditMessagingMintableBurnable } from "../interfaces/ICreditMessagingMintableBurnable.sol";
import { ICreditMessagingHandler, Credit } from "../interfaces/ICreditMessagingHandler.sol";

contract CreditMessagingMintableBurnable is CreditMessaging, ICreditMessagingMintableBurnable {
    constructor(address _endpoint, address _owner) CreditMessaging(_endpoint, _owner) {}

    // ---------------------------------- Only Owner ------------------------------------------

    /// @inheritdoc ICreditMessagingMintableBurnable
    ...
    function mintCredits(CreditBatch[] calldata _batches, string calldata _reason) external onlyOwner {
        if (bytes(_reason).length == 0) revert CreditMessagingMintableBurnable_EmptyReason();
