...
describe("OFTWrapperProxy:", function () {
    const chainIdSrc = 1
    const chainIdDst = 2
    ...
    let OFT, OFTProxy, MockToken, OFTProxySrc, MockTokenSrc, OFTDst, dstPath, srcPath, BP_DENOMINATOR, MAX_UINT
    ...
    before(async function () {
        ...
        OFTProxy = await ethers.getContractFactory("MockOFTProxy")
    ...
    beforeEach(async function () {
        ...
        OFTProxySrc = await OFTProxy.deploy(lzEndpointSrcMock.address, MockTokenSrc.address)
    ...
    it("sendProxyOFT()", async function () {
    ...
    it("sendProxyOFT() - amountToSwap < minAmount", async function () {
        let amountToMint = BigNumber.from("1000000000000000000000000")
        let amountToSwap = BigNumber.from("100000000000000")
        let defaultBps = 1
        let callerBps = 0
